...
import team.hnuwt.servicesoftware.simulation.service.ReadHandler;
import team.hnuwt.servicesoftware.simulation.service.SendHandler;
    ...
    public static void main(String[] args) throws UnknownHostException, IOException
    {
        ...
        while (!("c".equals(s) && !"C".equals(s)))
        {
            ...
            } else if ("d".equals(s) || "D".equals(s))
            {
                ...
                for (int i = 0; i < num; i++)
                    t[i] = new Thread(new SendHandler((long) i, meterNum, c[i].sc, 0));
                for (int i = 0; i < num; i++)
                    t[i].start();
            }
            System.out.println(
